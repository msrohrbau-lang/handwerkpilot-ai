(function(){
  ...
  function flush(){
    while(pending.length){
      ...
      try{fbq('track',item.event,item.params||{});}catch(e){}
    }
  }
  ...
  function showConsent(){
    ...
    document.getElementById('hpConsentReject').onclick=function(){localStorage.setItem(CONSENT_KEY,'no');pending.length=0;box.remove();};
  }
  ...
  window.hpTrack=function(event,params){
    ...
    if(consent==='yes'){
      ...
      pending.push({event:event,params:params||{}});
    }
  };
