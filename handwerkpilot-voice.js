...
let recognition=null,target=null,mode='command',lastActive=null;
...
function stop(){try{recognition?.stop()}catch{}const b=document.getElementById('hpVoiceFab');if(b)b.classList.remove('listening')}
function append(el,text){if(!el)return;const sep=el.value&&/textarea/i.test(el.tagName)?' ':'';el.value=(el.value||'')+sep+text;el.dispatchEvent(new Event('input',{bubbles:true}));el.dispatchEvent(new Event('change',{bubbles:true}));el.focus()}
function command(text){const t=text.toLowerCase().trim();
 ...
 if(t.includes('neuer bericht')||t.includes('bericht erstellen')){showView('bautage');setTimeout(()=>openBautagesbericht(),100);status('Neuer Bautagesbericht.');return}
 ...
 if(lastActive){append(lastActive,text);status('Text eingefügt.');return}
 status('Nicht verstanden: „'+text+'“');
}
function start(el=null){if(!SR){status('Spracherkennung wird von diesem Browser nicht unterstützt.',false);return}target=el||null;mode=target?'dictation':'command';recognition=new SR();recognition.lang='de-DE';recognition.interimResults=false;recognition.continuous=false;recognition.maxAlternatives=1;recognition.onstart=()=>{document.getElementById('hpVoiceFab')?.classList.add('listening');status(target?'Sprich jetzt …':'Sprachbefehl: z. B. „Neuer Bautagesbericht“',false)};recognition.onresult=e=>{const text=e.results?.[0]?.[0]?.transcript||'';if(target)append(target,text);else command(text)};recognition.onerror=e=>{const map={not_allowed:'Mikrofonzugriff nicht erlaubt.',no_speech:'Ich habe nichts gehört.',audio_capture:'Kein Mikrofon verfügbar.'};status(map[e.error]||'Spracherkennung konnte nicht gestartet werden.');};recognition.onend=()=>{document.getElementById('hpVoiceFab')?.classList.remove('listening');setTimeout(()=>document.getElementById('hpVoiceStatus')?.classList.add('hidden'),1600)};try{recognition.start()}catch(e){status('Spracherkennung ist bereits aktiv.')}}
