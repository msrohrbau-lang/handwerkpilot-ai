...
async function reloadOwnReports(){
  ...
  const existing=(documents||[]).filter(d=>d.document_type==='bautagesbericht'&&String(d.user_id)!==String(session.user.id));
  documents=[...other,...existing,...(data||[])];
...
function patchReportForm(){
  ...
  if(f){
    ...
    f.setAttribute('accept','image/*,application/pdf,.pdf');
    f.multiple=true;
    const label=f.previousElementSibling;
    if(label&&label.tagName==='LABEL')label.textContent='Fotos / PDF-Dateien von der Baustelle';
  }
}

function install(){
  const oldOpen=window.openBautagesbericht;
  if(oldOpen&&!oldOpen._hpBautageFix){
    const op=function(id=''){oldOpen(id);setTimeout(patchReportForm,40);setTimeout(patchReportForm,180)};
    op._hpBautageFix=1;window.openBautagesbericht=op;
  }
  const oldShow=window.showView;
  if(oldShow&&!oldShow._hpBautagePersistence){
    const sh=function(id){const r=oldShow(id);if(id==='bautage'&&isEmployee())setTimeout(reloadOwnReports,20);return r};
    sh._hpBautagePersistence=1;window.showView=sh;
  }
  if(isEmployee())reloadOwnReports();
}
